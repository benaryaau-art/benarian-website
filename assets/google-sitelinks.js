(() => {
  ...
  const LINKS=[['Hotel Deals','hotel-deals.html'],['Bali Entry Essentials','bali-travel-guide.html'],['Search Flights','flights.html'],['Contact Us','contact.html']];
  ...
  function installSeo(){document.title='BENARIAN | Luxury Hotels, Flights & Curated Travel';ensureMeta('meta[name="description"]',{name:'description',content:'Book luxury hotels, search flights and discover curated destinations, tours, restaurants, spa and wellness experiences with BENARIAN.'});ensureMeta('meta[name="robots"]',{name:'robots',content:'index,follow,max-image-preview:large,max-snippet:-1,max-video-preview:-1'});let c=document.head.querySelector('link[rel="canonical"]');if(!c){c=document.createElement('link');c.rel='canonical';document.head.appendChild(c)}c.href=BASE;let ld=document.head.querySelector('#benarian-priority-links');if(!ld){ld=document.createElement('script');ld.id='benarian-priority-links';ld.type='application/ld+json';document.head.appendChild(ld)}ld.textContent=JSON.stringify({'@context':'https://schema.org','@type':'ItemList',name:'BENARIAN Travel Services',itemListElement:LINKS.map(([name,path],index)=>({'@type':'ListItem',position:index+1,name,url:BASE+path.replace('.html','')}))});}
