...
window.BENARIAN_PARTNERS = {
  ...
  agoda: { enabled: false, affiliateBaseUrl: "" }, trip: { enabled: false, affiliateBaseUrl: "" }, expedia: { enabled: false, affiliateBaseUrl: "" }, hotels: { enabled: false, affiliateBaseUrl: "" }, getYourGuide: { enabled: false, affiliateBaseUrl: "" }
};
...
function initialiseBenarianRuntime() {
  ...
  if (hero && !hero.querySelector('.benarian-hero-video') && !reducedMotion && !saveData) {
    const loadHeroVideo = () => {
      ...
      video.className = 'benarian-hero-video'; video.autoplay = true; video.muted = true; video.loop = true; video.playsInline = true; video.preload = 'none'; video.setAttribute('aria-hidden', 'true');
      video.innerHTML = '<source src="assets/videos/Video.mp4" type="video/mp4">'; hero.prepend(video);
      const overlay = document.createElement('span'); overlay.className = 'benarian-hero-overlay'; overlay.setAttribute('aria-hidden', 'true'); hero.insertBefore(overlay, video.nextSibling); video.play().catch(() => {});
    };
    const scheduleVideo = () => { if ('requestIdleCallback' in window) window.requestIdleCallback(loadHeroVideo, { timeout: 1800 }); else window.setTimeout(loadHeroVideo, 500); };
    if (document.readyState === 'complete') scheduleVideo(); else window.addEventListener('load', scheduleVideo, { once: true });
  }
