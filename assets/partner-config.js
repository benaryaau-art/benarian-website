...
function initialiseBenarianRuntime() {
  ...
  if (homeHotels) {
    ...
    homeHotels.innerHTML = `
      <div class="benarian-home-collection-head">
        ...
        <span>A restrained selection from our Expedia Travel Shop. Live rates, availability and verified guest information are shown on Expedia.</span>
      </div>
      <div class="benarian-home-collection-list">
        ${BENARIAN_EXPEDIA_HOTELS.slice(0, 4).map((hotel, index) => `
          <a href="${expediaShop}" target="_blank" rel="noopener sponsored" class="benarian-home-hotel-row">
            ...
            <span class="benarian-home-hotel-arrow">VIEW LIVE PRICE →</span>
          </a>`).join('')}
      </div>
  ...
  if (hotelGrid) {
    ...
    hotelGrid.innerHTML = BENARIAN_EXPEDIA_HOTELS.map((hotel, index) => `
      <article class="benarian-text-hotel-card">
        ...
        <a href="${expediaShop}" target="_blank" rel="noopener sponsored">VIEW LIVE PRICE ON EXPEDIA →</a>
      </article>`).join('');
  ...
  document.querySelectorAll('.header .nav').forEach(nav => {
    if (!nav.querySelector('a[data-benarian-expedia-shop]')) {
      ...
      link.href = expediaShop; link.target = '_blank'; link.rel = 'noopener sponsored'; link.textContent = 'EXPEDIA SHOP'; link.setAttribute('data-benarian-expedia-shop', 'true'); nav.appendChild(link);
    }
  });
